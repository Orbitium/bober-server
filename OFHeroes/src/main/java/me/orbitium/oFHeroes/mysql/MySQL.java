package me.orbitium.oFHeroes.mysql;
...
public class MySQL {
    ...
    public static void connect(boolean force) throws SQLException {
        if ((connection == null || connection.isClosed()) || force) {
            FileConfiguration config = OFHeroes.instance.getConfig();
    ...
    public static Connection getConnection() {
        ...
        while (retries < MAX_RETRIES) {
            try {
                connect(true);
                if (connection != null) {
                    OFHeroes.instance.getLogger().log(Level.INFO, "Connected to the database successfully on attempt " + (retries + 1));
                    return connection;
                }
            } catch (SQLException e) {
                retries++;
                OFHeroes.instance.getLogger().log(Level.INFO, "Failed to connect. Retrying attempt " + retries + " of " + MAX_RETRIES);
